...
var Terminal = Terminal || function(containerId) {
	...
	var worker_ = new Worker('js/worker.js');
	...
	function processNewCommand_(e) {
		...
		if (e.keyCode == 9) { // Tab
			...
			switch (cmd) {
				case 'echo': 
					output(this.value.replace(cmd + ' ', ''));
					break;
				case 'js':
					var input = this.value.replace(cmd + ' ', '');
					var inputjs = args.join(' ');
					if (!inputjs) {
						output('usage: ' + cmd + ' code');
						break;
					}
						...
						if (entries.length) {
							...
							util.toArray(entries).forEach(function(entry, i) {
								html.push(
										...
										'">', entry.name, entry.isDirectory ? '/': '', '</span><br>');
							});
							html.push('</div>');
							output(html.join(''));
						}
				...
				case 'rm':
					...
					var recursive = false;
					['-r', '-f', '-rf', '-fr', '-Rf'].forEach(function(arg, i) {
	...
	function toggle3DView_() {
		...
		if (body.classList.contains('offscreen')) {
			...
			var transEnd_ = function(e) {
				...
				iframe.id = 'fsn';
				iframe.src = 'http://www.htmlfivewow.com/demos/fsn/fsn_proto.html';
		...
		initFS: function(persistent, size) {
			...
			window.requestFileSystem(type, size, function(filesystem) {
				...
				cwd_.getDirectory('testquotaforfsfolder', {create: true}, function(dirEntry) {
					dirEntry.remove(function() { console.log('File removed.'); }); // If successfully created, just delete it.
				}, function(e) {
